...
int main(){
    char inputname[101],outputname[101];
    char l[2],r[2];
    int num;
    WAVheader w;
    int k;
    ...
    int s=w.subblock2_size/w.byterate,currpos=sizeof(WAVheader);
    int bytepersecond=w.byterate;
    if(s<k){
        for(;fread(l,sizeof(char),2,fpir)>0&&fread(r,sizeof(char),2,fpir);){ 
            fwrite(r,sizeof(char),2,fpow);
            fwrite(l,sizeof(char),2,fpow);
        }
    }else{
        for(int i=0;i<k*(bytepersecond/4);i++){
            fread(l,sizeof(char),2,fpir);
            fread(r,sizeof(char),2,fpir);
            fwrite(r,sizeof(char),2,fpow);
            fwrite(l,sizeof(char),2,fpow);
        }
        for(;fread(l,sizeof(char),2,fpir)>0&&fread(r,sizeof(char),2,fpir)>0;){
            fwrite(l,sizeof(char),2,fpow);
            fwrite(r,sizeof(char),2,fpow);
        }
        fclose(fpow);
